...
namespace QLTOUR.GiaoDIen
{
     public partial class Ve : Form
     {
          ...
          public Ve(string maTour, string manv)
          {
               ...
               MaNV = manv;
               MaTour = maTour;
               string chuoitruyvan = "SELECT MaSan,TenSan,MoTaSan,AnhSan,GiaSan,TGBatDau,TGKetThuc,lt.TenLoaisan FROM LOAISAN lt JOIN THONGTINSAN ttt ON lt.MaLoaisan = ttt.MaLoaisan WHERE MaSan = '" + maTour+"'";
               DataTable dt = db.getDataTable(chuoitruyvan);
               if (dt.Rows.Count > 0)
               {
                    DataRow row = dt.Rows[0]; // Lấy dòng đầu tiên của kết quả truy vấn

                    lb_matour.Text = row["MaSan"].ToString();
                    lb_tentour.Text = row["TenSan"].ToString();
                    lb_giatour.Text = ((Decimal)row["GiaSan"]).ToString("");
          ...
          private void btn_kiemtraKH_Click(object sender, EventArgs e)
          {
               ...
               else
               {
                    ...
                    {
                         DialogResult r;
                         r = MessageBox.Show("Chưa có KH này. Bạn phải thêm khách hàng trước khi đặt sân ?", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
